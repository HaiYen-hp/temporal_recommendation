...
def embedding(inputs,
    ...
    with tf.compat.v1.variable_scope(scope, reuse=reuse):
        lookup_table = tf.compat.v1.get_variable(name='lookup_table',
                                       ...
                                       initializer=tf.compat.v1.truncated_normal_initializer(mean=0.01, stddev=0.02),
...
def noam_scheme(init_lr, global_step, warmup_steps=4000.):
    ...
    '''
    step = tf.cast(global_step + 1, dtype=tf.float64)
...
def get_shape_list(tensor, expected_rank=None, name=None):
    ...
    if expected_rank is not None:
      tf.debugging.assert_rank(tensor, expected_rank, name)
